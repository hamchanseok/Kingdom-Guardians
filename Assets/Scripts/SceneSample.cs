...
public class SceneSample : MonoBehaviour
{
    ...
    public void OnClick_TankChange ()
    {
        ...
        userSession.m_nUserData[0] = (byte)UnityEngine.Random.Range(0, 2);
    ...
    void RoomOneUserAdd(UserSession user)
    {
        ...
        playerObj.GetComponent<PlayerManager>().Init(user);
    }
            ...
            if (roomSession.m_userList[i].m_szUserID == user.m_szUserID)
            {
                ...
                if (playerObj)
                {
                    ...
                    playerObj.transform.position =
                                Vector3.MoveTowards(playerObj.transform.position, roomSession.m_userList[i].m_userTransform[0].GetVector3(), moveSpeed*Time.deltaTime);
